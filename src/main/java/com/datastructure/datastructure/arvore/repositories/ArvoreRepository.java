package com.datastructure.datastructure.arvore.repositories;
...
public class ArvoreRepository {
    ...
    public boolean remover(long v) throws ValueNotFoundException {
        if (root == null)
            throw new ValueNotFoundException("Nao Encontrado no metodo remover"); // se arvore vazia
    ...
    public List<Long> obterValores() {
        List<Long> valores = new ArrayList<>();
        obterValoresRecursivamente(root, valores);
        return valores;
    }

    private void obterValoresRecursivamente(No atual, List<Long> valores) {
        if (atual != null) {
            obterValoresRecursivamente(atual.esq, valores);
            valores.add(atual.item);
            obterValoresRecursivamente(atual.dir, valores);
        }
    }
    ...
    public List<Long> posOrder() {
        List<Long> valores = new ArrayList<>();
        posOrder(root, valores);
        return valores;
    }

    private void preOrder(No atual, List<Long> valores) {
        if (atual != null) {
            valores.add(atual.item);
            preOrder(atual.esq, valores);
            preOrder(atual.dir, valores);
        }
    }

    private void inOrder(No atual, List<Long> valores) {
        if (atual != null) {
            inOrder(atual.esq, valores);
            valores.add(atual.item);
            inOrder(atual.dir, valores);
        }
    }

    private void posOrder(No atual, List<Long> valores) {
        if (atual != null) {
            posOrder(atual.esq, valores);
            posOrder(atual.dir, valores);
            valores.add(atual.item);
        }
    }
